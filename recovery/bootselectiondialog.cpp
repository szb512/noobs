...
BootSelectionDialog::BootSelectionDialog(const QString &defaultPartition,
                                         QWidget *parent)
    : QDialog(parent), _countdown(11), ui(new Ui::BootSelectionDialog) {
  ...
  if (QProcess::execute("mount -t ext4 -o ro " SETTINGS_PARTITION
                        " /settings") != 0) {
    QMessageBox::critical(this, tr("Cannot display boot menu"),
  ...
  QVariantList installed_os =
      Json::loadFromFile("/settings/installed_os.json").toList();
  ...
  if (ui->list->count() != 0) {
    ...
    int partition =
        settings.value("default_partition_to_boot", defaultPartition).toInt();

    if (partition != 800) {
      // Start timer
      qDebug() << "Starting 10 second timer before booting into partition"
               << partition;
...
BootSelectionDialog::~BootSelectionDialog() { delete ui; }

void BootSelectionDialog::bootPartition() {
  QSettings settings("/settings/noobs.conf", QSettings::IniFormat, this);
  QByteArray partition =
      settings.value("default_partition_to_boot", 800).toByteArray();
  qDebug() << "Booting partition" << partition;
  putFileContents("/sys/module/bcm2708/parameters/reboot_part",
                  partition + "\n");
  QDialog::accept();
}
...
void BootSelectionDialog::on_list_activated(const QModelIndex &) { accept(); }
...
void BootSelectionDialog::countdown() {
  setWindowTitle(
      tr("Previously selected OS will boot in %1 seconds").arg(--_countdown));
  if (_countdown == 0) {
    _timer.stop();
    bootPartition();
  }
