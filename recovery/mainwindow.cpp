...
MainWindow::MainWindow(const QString &defaultDisplay, QSplashScreen *splash,
                       QWidget *parent)
    : QMainWindow(parent), ui(new Ui::MainWindow), _qpd(NULL), _kcpos(0),
      _defaultDisplay(defaultDisplay), _silent(false), _allowSilent(false),
      _splash(splash), _settings(NULL), _activatedEth(false),
      _numInstalledOS(0), _netaccess(NULL), _displayModeBox(NULL) {
  ui->setupUi(this);
  ...
  if (qApp->arguments().contains("-runinstaller") && !_partInited) {
    ...
    _qpd->setWindowFlags(Qt::Window | Qt::CustomizeWindowHint |
                         Qt::WindowTitleHint);

    InitDriveThread *idt = new InitDriveThread(this);
    connect(idt, SIGNAL(statusUpdate(QString)), _qpd,
    ...
    connect(idt, SIGNAL(query(QString, QString, QMessageBox::StandardButton *)),
            this,
            SLOT(onQuery(QString, QString, QMessageBox::StandardButton *)),
...
void MainWindow::populate() {
  ...
  if (!_allowSilent) {
    _qpd = new QProgressDialog(tr("Please wait while NOOBS initialises"),
                               QString(), 0, 0, this);
    _qpd->setWindowFlags(Qt::Window | Qt::CustomizeWindowHint |
                         Qt::WindowTitleHint);
    _qpd->show();
  ...
  if (QFile::exists(SETTINGS_PARTITION)) {
    /* Try mounting read-only first, if fails try read-write as it may recover
     * from journal */
    if (QProcess::execute("mount -o remount,ro /settings") != 0 &&
        QProcess::execute("mount -o remount,rw /settings") != 0) {
      if (QMessageBox::question(
              this, tr("Error mounting settings partition"),
        ...
        if (QProcess::execute("/usr/sbin/mkfs.ext4 " SETTINGS_PARTITION) != 0 ||
          ...
          QMessageBox::critical(this, tr("Reformat failed"),
                                tr("SD card might be damaged"),
                                QMessageBox::Close);
        }
    ...
    _settings =
        new QSettings("/settings/noobs.conf", QSettings::IniFormat, this);
  ...
  repopulate();
  _availableMB = (getFileContents("/sys/class/block/mmcblk0p3/start")
                      .trimmed()
                      .toULongLong() -
                  getFileContents("/sys/class/block/mmcblk0p2/start")
                      .trimmed()
                      .toULongLong()) /
                 2048;
  updateNeeded();
  ...
  if (ui->list->count() != 0) {
    QList<QListWidgetItem *> l =
        ui->list->findItems(RECOMMENDED_IMAGE, Qt::MatchExactly);
...
void MainWindow::repopulate() {
  ...
  foreach (QVariant v, images.values()) {
    ...
    if (QFile::exists(iconFilename)) {
      ...
      if (avs.isEmpty()) {
        ...
        if (iconsize.width() > currentsize.width() ||
            iconsize.height() > currentsize.height()) {
          /* Make all icons as large as the largest icon we have */
          currentsize = QSize(qMax(iconsize.width(), currentsize.width()),
                              qMax(iconsize.height(), currentsize.height()));
  ...
  if (haveicons) {
    /* Giving items without icon a dummy icon to make them have equal height and
     * text alignment */
...
QMap<QString, QVariantMap> MainWindow::listImages() {
  ...
  foreach (QString image, list) {
    ...
    if (QFile::exists(imagefolder + "/flavours.json")) {
      QVariantMap v =
          Json::loadFromFile(imagefolder + "/flavours.json").toMap();
  ...
  if (_settings) {
    ...
    foreach (QVariant v, i) {
      ...
      if (m.value("release_date").toString() >
          images[name].value("release_date").toString()) {
        images[name] = m;
      }
  ...
  for (QMap<QString, QVariantMap>::iterator i = images.begin();
       i != images.end(); i++) {
    if (!i.value().contains("nominal_size")) {
      /* Calculate nominal_size based on information inside partitions.json */
      int nominal_size = 0;
      QVariantMap pv = Json::loadFromFile(i.value().value("folder").toString() +
                                          "/partitions.json")
                           .toMap();
...
void MainWindow::on_actionWrite_image_to_disk_triggered() {
  ...
  if (_silent ||
      QMessageBox::warning(
          this, tr("Confirm"),
          tr("Warning: this will install the selected Operating System(s). All "
             "existing data on the SD card will be overwritten, including any "
             "OSes that are already installed."),
          QMessageBox::Yes, QMessageBox::No) == QMessageBox::Yes) {
    setEnabled(false);
    ...
    foreach (QListWidgetItem *item, selected) {
      ...
      if (!entry.contains("folder")) {
        ...
        downloadMetaFile(entry.value("os_info").toString(),
                         folder + "/os.json");
        downloadMetaFile(entry.value("partitions_info").toString(),
                         folder + "/partitions.json");

        if (entry.contains("marketing_info"))
          downloadMetaFile(entry.value("marketing_info").toString(),
                           folder + "/marketing.tar");

        if (entry.contains("partition_setup"))
          downloadMetaFile(entry.value("partition_setup").toString(),
                           folder + "/partition_setup.sh");

        if (entry.contains("icon"))
          downloadMetaFile(entry.value("icon").toString(),
                           folder + "/icon.png");
      }
    }
    ...
    if (_numMetaFilesToDownload == 0) {
      ...
      _qpd = new QProgressDialog(tr("The install process will begin shortly."),
                                 QString(), 0, 0, this);
      _qpd->setWindowFlags(Qt::Window | Qt::CustomizeWindowHint |
                           Qt::WindowTitleHint);
      _qpd->show();
...
void MainWindow::on_actionCancel_triggered() { close(); }
...
void MainWindow::onCompleted() {
  ...
  if (!_silent)
    QMessageBox::information(this, tr("OS(es) installed"),
...
void MainWindow::onQuery(const QString &msg, const QString &title,
                         QMessageBox::StandardButton *answer) {
  _qpd->hide();
  *answer = QMessageBox::question(this, title, msg,
...
void MainWindow::on_list_currentRowChanged() {
  ...
  ui->actionEdit_config->setEnabled(
      item && item->data(Qt::UserRole).toMap().contains("partitions") &&
      item->data(Qt::UserRole).toMap().value("bootable", true) == true);
}

void MainWindow::update_window_title() {
  setWindowTitle(QString(tr("NOOBS v%1 - Built: %2"))
                     .arg(VERSION_NUMBER, QString::fromLocal8Bit(__DATE__)));
}
...
void MainWindow::displayMode(int modenr, bool silent) {
  ...
  if (!silent && _displayModeBox) {
    /* User pressed another mode selection key while the confirmation box is
     * being displayed */
    silent = true;
  ...
  update->start(QString(
      "sh -c \"tvservice -s | cut -d , -f 2 | cut -d \' \' -f 2 | cut -d x -f "
      "1;tvservice -s | cut -d , -f 2 | cut -d \' \' -f 2 | cut -d x -f 2\""));
  ...
  qDebug() << "Current overscan"
           << "top" << oTop << "bottom" << oBottom << "left" << oLeft << "right"
           << oRight;
  QScreen::instance()->setMode(xres - oLeft - oRight, yres - oTop - oBottom,
                               16);
  ...
  ld->setGeometry(
      QStyle::alignedRect(Qt::LeftToRight, Qt::AlignHCenter | Qt::AlignBottom,
                          ld->size(), qApp->desktop()->availableGeometry()));
  ...
  led_blink->start("sh -c \"echo 1 > /sys/class/leds/led0/brightness; sleep 3; "
                   "echo 0 > /sys/class/leds/led0/brightness\"");

  // Inform user of resolution change with message box.
  if (!silent && _settings) {
    _displayModeBox =
        new QMessageBox(QMessageBox::Question, tr("Display Mode Changed"),
    ...
    if (_displayModeBox->standardButton(_displayModeBox->clickedButton()) ==
        QMessageBox::Yes) {
      remountSettingsRW();
...
bool MainWindow::eventFilter(QObject *, QEvent *event) {
  if (event->type() == QEvent::KeyPress) {
    ...
    // experimentally by using keys 1-4. NOOBS will default to using HDMI
    // preferred mode.
...
void MainWindow::on_actionEdit_config_triggered() {
  ...
  if (item && item->data(Qt::UserRole).toMap().contains("partitions")) {
    QVariantList l =
        item->data(Qt::UserRole).toMap().value("partitions").toList();
    if (!l.isEmpty())
...
void MainWindow::on_actionBrowser_triggered() { startBrowser(); }

bool MainWindow::requireNetwork() {
  if (!QFile::exists("/tmp/resolv.conf")) {
    QMessageBox::critical(
        this, tr("No network access"),
        tr("Wired network access is required for this feature. Please insert a "
           "network cable into the network port."),
        QMessageBox::Close);
    return false;
  }
...
void MainWindow::startNetworking() {
  ...
  /* Try enabling interface twice as sometimes it times out before getting a
   * DHCP lease */
...
void MainWindow::rebuildInstalledList() {
  ...
  for (int i = 5; i <= MAXIMUM_PARTITIONS; i++) {
    ...
    if (QFile::exists(part) &&
        QProcess::execute("mount -t vfat " + part + " /mnt2") == 0) {
...
void MainWindow::downloadListComplete() {
  ...
  if (reply->error() != reply->NoError || httpstatuscode < 200 ||
      httpstatuscode > 399) {
    _qpd->hide();
    QMessageBox::critical(
        this, tr("Download error"),
        tr("Error downloading distribution list from Internet"),
        QMessageBox::Close);
  } else {
...
void MainWindow::processJson(QVariant json) {
  if (json.isNull()) {
    QMessageBox::critical(this, tr("Error"),
                          tr("Error parsing list.json downloaded from server"),
                          QMessageBox::Close);
    return;
  }
  ...
  foreach (QVariant osv, list) {
      ...
      foreach (QVariant flv, flavours) {
        ...
        if (!alreadyHasItem(name, item.value("release_date"))) {
          ...
          QListWidgetItem *witem =
              searchForOlderItem(name, item.value("release_date"));

          if (witem) {
            /* Older version is locally available. Replace info with newer
             * Internet version */
            item.insert(
                "installed",
                witem->data(Qt::UserRole).toMap().value("installed", false));
    ...
    if (os.contains("description")) {
      ...
      if (!alreadyHasItem(name, os.value("release_date"))) {
        ...
        QListWidgetItem *witem =
            searchForOlderItem(name, os.value("release_date"));
  ...
  if (_numIconsToDownload) {
    foreach (QString iconurl, iconurls) { downloadIcon(iconurl, iconurl); }
  } else {
    if (_qpd) {
      _qpd->deleteLater();
      _qpd = NULL;
...
void MainWindow::downloadIcon(const QString &urlstring,
...
bool MainWindow::alreadyHasItem(const QVariant &name,
                                const QVariant &releasedate) {
  for (int i = 0; i < ui->list->count(); i++) {
    QVariantMap m = ui->list->item(i)->data(Qt::UserRole).toMap();
    if (m.value("name").toString() == name.toString() &&
        m.value("release_date").toString() >= releasedate.toString()) {
      return true;
    }
  }
...
QListWidgetItem *MainWindow::searchForOlderItem(const QVariant &name,
                                                const QVariant &releasedate) {
  for (int i = 0; i < ui->list->count(); i++) {
...
void MainWindow::downloadIconComplete() {
  ...
  if (reply->error() != reply->NoError || httpstatuscode < 200 ||
      httpstatuscode > 399) {
    // QMessageBox::critical(this, tr("Download error"), tr("Error downloading
    // icon '%1'").arg(reply->url().toString()), QMessageBox::Close);
...
void MainWindow::updateNeeded() {
  ...
  foreach (QListWidgetItem *item, selected) {
    ...
    if (entry.value("name").toString().contains("risc", Qt::CaseInsensitive)) {
      /* RiscOS needs to start at a predetermined sector, calculate the extra
       * space needed for that */
      int startSector = getFileContents("/sys/class/block/mmcblk0p2/start")
                            .trimmed()
                            .toULongLong();
      _neededMB += (RISCOS_SECTOR_OFFSET - startSector) / 2048;
    }
    if (entry.value("name").toString().contains("data partition",
                                                Qt::CaseInsensitive)) {
      datapartition = true;
    }
  }

  ui->neededLabel->setText(
      QString("%1: %2 MB").arg(tr("Needed"), QString::number(_neededMB)));
  ui->availableLabel->setText(
      QString("%1: %2 MB").arg(tr("Available"), QString::number(_availableMB)));
...
void MainWindow::on_list_itemChanged(QListWidgetItem *) { updateNeeded(); }

void MainWindow::downloadMetaFile(const QString &urlstring,
...
void MainWindow::downloadMetaComplete() {
  ...
  if (reply->error() != reply->NoError || httpstatuscode < 200 ||
    ...
    QMessageBox::critical(this, tr("Download error"),
                          tr("Error downloading meta file") + "\n" +
                              reply->url().toString(),
                          QMessageBox::Close);
    setEnabled(true);
  } else {
    ...
    if (f.write(reply->readAll()) == -1) {
      QMessageBox::critical(this, tr("Download error"),
                            tr("Error writing downloaded file to SD card. SD "
                               "card or file system may be damaged."),
                            QMessageBox::Close);
      setEnabled(true);
    } else {
      _numMetaFilesToDownload--;
...
void MainWindow::startImageWrite() {
  /* All meta files downloaded, extract slides tarball, and launch image writer
   * thread */
  ...
  foreach (QListWidgetItem *item, selected) {
    ...
    if (entry.contains("folder")) {
      ...
      QVariantMap json =
          Json::loadFromFile(folder + "/partitions.json").toMap();
  ...
  connect(imageWriteThread, SIGNAL(parsedImagesize(qint64)), _qpd,
          SLOT(setMaximum(qint64)));
  connect(imageWriteThread, SIGNAL(completed()), this, SLOT(onCompleted()));
  connect(imageWriteThread, SIGNAL(error(QString)), this,
          SLOT(onError(QString)));
  connect(imageWriteThread, SIGNAL(statusUpdate(QString)), _qpd,
          SLOT(setLabelText(QString)));
  connect(imageWriteThread, SIGNAL(runningMKFS()), _qpd,
          SLOT(pauseIOaccounting()), Qt::BlockingQueuedConnection);
  connect(imageWriteThread, SIGNAL(finishedMKFS()), _qpd,
          SLOT(resumeIOaccounting()), Qt::BlockingQueuedConnection);
...
void MainWindow::hideDialogIfNoNetwork() {
  if (_qpd) {
    QByteArray carrier =
        getFileContents("/sys/class/net/eth0/carrier").trimmed();
    if (carrier != "1") {
      ...
      if (ui->list->count() == 0) {
        ...
        QMessageBox::critical(
            this, tr("No network access"),
            tr("Wired network access is required to use NOOBS without local "
               "images. Please insert a network cable into the network port."),
            QMessageBox::Close);
      }
    }
  }
